...
export function Navbar() {
    return (
        <header className="sticky top-0 z-50 border-b border-white/5 bg-slate-950/60 backdrop-blur-md">
            <div className="mx-auto flex w-[90%] max-w-300 items-center justify-between py-4">
                ...
                <nav className="flex gap-8">
                    {links.map((link) => (
                        <a
                            key={link.name}
                            href={link.href}
                            className="group relative font-heading text-sm text-gray-200 transition-colors duration-300 hover:text-orange-400"
                        >
                            {link.name}
